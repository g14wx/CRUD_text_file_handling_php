	...
	if($_POST['retrieve'] == "1") {
		...
		if($file = fopen($_POST['fileName'],"w")) {
			...
			if(fwrite($file,$_POST['content'])) {
				...
				die(json_encode(["desc"=>"Almacenado.","level"=>"success"]));
				
			} else { die(json_encode(["desc"=>"Error al escribir en el archivo","level"=>"fatal"])); } 
			
		} else { die(json_encode(["desc"=>"No se pudo abrir el archivo","level"=>"fatal"])); }
		
	}
	
	else {
		if(!isset($_GET['fileName'])) { die("No hay archivo seleccionado, asegurese que lo este enviando como url parameter ?fileName=nombrearchivo.txt"); }
	...
	<body>
		...
		<div id = 'menubar'>
			File: <a href = '#' target = '_BLANK' id = 'fileLink'></a>
			 | 
			<input type = 'button' onclick = 'doSave();' value = 'Almacenar' style = 'padding:0px;'>
			 / 
